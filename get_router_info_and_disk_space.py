...
import telnetlib, time, re, getpass, threading, os, netsnmp
...
def threadFunction(ip, target, error_file):
	...
	time.sleep(2)	
	try:
		total_flash = int(snmp_get(ip, FLASH_TOTAL_OID))/1024
		free_flash = int(snmp_get(ip, FLASH_FREE_OID))/1024
	except:
		err = ip + '\tOID Error\n'
		error_file.write(err)
	bwduplex = bw + '\\' + duplex
	...
	report = "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s"%(sitecode, hostname, total_flash, free_flash, bwduplex, model, ver, loopback, wanIP, ipInterfaces)
	target.write(report)
	target.write("\n")
	tn.close()
...
if __name__ == "__main__":
	...
	allThreads = []
	outputfile = 'router_output/router_output_' + datetime.now().strftime('%d%m%Y')
	with open(outputfile, 'w') as target:
		error_file = open('error_file', 'w')
		target.write('Site Code,Hostname,Total Space,Free Space,BW,Duplex,Model,Version,Loopback IP,WAN IP,LAN IPs\n')
		with open('failed_router_ips2') as f:
			for ip in f:
				ip = ip.strip()
